...
# 16
# SYSROOT_INCLUDE_FOLDER = "/Volumes/disk/applications/DevEco-Studio_5.0.11.110.app/Contents/sdk/default/openharmony/native/sysroot/usr/include"
# 17
# SYSROOT_INCLUDE_FOLDER = "/Volumes/disk/artifact/command-line-tools-5.0.13.240-17/sdk/default/openharmony/native/sysroot/usr/include"
# 20
SYSROOT_INCLUDE_FOLDER = "/Volumes/disk/applications/DevEco-Studio_6.0.0.858.app/Contents/sdk/default/openharmony/native/sysroot/usr/include"
...
def parse_function_prototypes_with_ctags(file_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        cmd = ['/opt/homebrew/bin/ctags', '--kinds-c=p', '--fields=+S', '-n', '-f', '-', str(file_path)]
        ...
        for line in result.stdout.strip().split('\n'):
            ...
            if not signature:
                signature = extract_signature_from_file(file_path, function_name, line_no)
...
def extract_signature_from_file(file_path: Path, function_name: str, line_no: int) -> str:
    ...
    try:
        ...
        for i in range(start_line, min(len(lines), start_line + 20)):  # Look at most 20 lines ahead
            line = lines[i].strip()
